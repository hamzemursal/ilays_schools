...
const COMMON_REASONS = [
  ...
  "One or more marks need correction",
  "Wrong exam paper attached",
  "Other",
];

// A reason is always required — ExamsService.unpublishSubmission rejects the
// request outright without one — same shape as ReturnForCorrectionDialog,
// since this also removes something the Student/Parent Portals were already
// showing.
export function UnpublishResultsDialog({
  open,
  loading,
  onConfirm,
  onCancel,
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="font-semibold text-foreground">Undo Publish?</h2>
            <p className="mt-1 text-sm text-foreground-soft">
              These results will immediately stop being visible to students and parents. A reason is required.
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-3">
          ...
          {preset === "Other" && (
            <Textarea
              ...
              placeholder="e.g. Needs to be reviewed again before release."
              rows={3}
              autoFocus
            />
          )}
